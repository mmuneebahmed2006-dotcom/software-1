...
import { ChevronLeft, ChevronRight, FilePlus2, FileText, Folder, FolderPlus, Pencil, Search, Trash2 } from "lucide-react";
...
export function SavedDocumentsSidebar({ documents, activeId, folders = [], activeFolder = "General", onSelectFolder = () => {}, onNew, onOpen, onRename, onDelete, onCreateFolder }: Props) {
  ...
  }, [documents, query, range, from, to, folder]);
  return <aside className={`no-print saved-sidebar ${collapsed ? "collapsed" : ""}`}>
    ...
    {collapsed ? <Button type="button" size="icon" onClick={onNew} aria-label="New document"><FilePlus2 size={18}/></Button> : <>
      ...
      <div className="folder-chips"><button type="button" className={folder === "all" ? "active" : ""} onClick={() => setFolder("all")}>All folders</button>{folderNames.map((name) => <button type="button" key={name} className={`${folder === name ? "active" : ""} ${activeFolder === name ? "current" : ""}`} onClick={() => { setFolder(name); onSelectFolder(name) }}><Folder size={13}/> {name} <small>{documents.filter((d) => d.folder === name).length}</small></button>)}</div>
