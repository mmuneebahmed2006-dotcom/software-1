...
export function CompanyDetailsDialog({ details, onSave }: Props) {
  ...
  const field = (key: keyof CompanyDetails, label: string) => <label className="dialog-field"><span>{label}</span><Input value={draft[key]} onChange={(event) => setDraft((current) => ({ ...current, [key]: event.target.value }))}/></label>;
